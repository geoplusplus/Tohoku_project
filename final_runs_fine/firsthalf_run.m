...
% optional : plot the LB and UB 
numslip = (size(UB,1)-10)/2; 
figure; subplot(221)
patch(p(trired'),q(trired'),repmat(LB(11:10+numslip)',3,1)); axis equal
colorbar
subplot(222)
patch(p(trired'),q(trired'),repmat(UB(11:10+numslip)',3,1)); axis equal
colorbar
subplot(223)
patch(p(trired'),q(trired'),repmat(LB(11+numslip:end)',3,1)); axis equal
colorbar
subplot(224)
patch(p(trired'),q(trired'),repmat(UB(11+numslip:end)',3,1)); axis equal
colorbar
...
load samples_taken1_1.mat
load samples_taken1_2.mat 
stage = 0; num = ceil(rand*1000) 
opt.target = posterior; 
post = exp(logpst); 
samplestage = smplstage0; 

tic; outsmpl = ATMIP_remaining1(opt,beta,post,samplestage,stage,num);  
b = toc
